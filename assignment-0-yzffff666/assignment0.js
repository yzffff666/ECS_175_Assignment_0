...
class Shape
{
    ...
    createVAO( gl, shader )
    {
        ...
        // Get the location of the attribute in shader
        const a_position_location = gl.getAttribLocation(shader.program, 'a_position');
        console.log(a_position_location, "a_location");
        ...
        gl.vertexAttribPointer(a_position_location, this.num_components, gl.FLOAT, false, 0, 0);
    ...
    createVBO( gl )
    {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.vertices_buffer); // bindBuffer(target, buffer)
        // bufferData(target, data, type) use Float32Array for VBOs
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.vertices), gl.STATIC_DRAW);
    ...
    createIBO( gl )
    {
        ...
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.index_buffer); // bindBuffer(target, buffer)

        // bufferData(target, data, type) Uint16Array for IBOs
    ...
    render( gl )
    {
        ...
        // TODO: Bind vertex array object (VAO)
        gl.bindVertexArray(this.vertex_array_object);

        // TODO: Bind index buffer object (IBO)
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.index_buffer);
...
class Triangle extends Shape
{
    ...
    constructor(gl, shader, position, color, sideLength) 
    {
        ...
        let cosangle = Math.cos(deg2rad(60))
        let sinangle = Math.sin(deg2rad(60))
        ...
        let vertices = [
            ...
            // Side2 
            // Side3
            position[0], position[1],                                                     // First vertex (top)
            (position[0] - sideLength * cosangle), (position[1] + sideLength * sinangle), // Second vertex (bottom left)
            (position[0] + sideLength * cosangle), (position[1] + sideLength * sinangle)  // Third vertex (bottom right)
        ]
...
class WebGlApp 
{
    ...
    initGl( )
    {
        ...
        const gl = canvas.getContext('webgl2'); // 2 dimensional
    ...
    clearCanvas( gl )
    {
        ...
        const rgb = hex2rgb("#022851"); // return an array: [r, g, b]    

        gl.clearColor(rgb[0], rgb[1], rgb[2], 1.0);    

        gl.clear(gl.COLOR_BUFFER_BIT); // clear 2D buffer
    }
    ...
    addTriangle( gl, shader, position, sideLength )
    {
        ...
        const triangle = new Triangle(gl, shader, position, aggieGold, sideLength);
